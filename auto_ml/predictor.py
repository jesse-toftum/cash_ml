...
class Predictor(object):
    ...
    def __init__(self, type_of_estimator, column_descriptions, verbose=True, name=None):
        ...
        self._scorer = None
        self.date_cols = []
        # Later on, if this is a regression problem, we will possibly take the natural log of our
        #  y values for training, but we will still want to return the predictions in their
        # normal scale (not the natural log values)
    ...
    def _validate_input_col_descriptions(self):
        ...
        for key, value in self.column_descriptions.items():
            ...
            if value == 'output':
                ...
                raise ValueError(
                    'We are not sure how to process this column of data: ' + str(value) +
                    '. Please pass in "output", "categorical", "ignore", "nlp", or "date".')
    ...
    def _construct_pipeline(self,
                            ...
                            final_model_step_name='final_model',
        ...
        if self.user_input_func is not None:
            if trained_pipeline is not None:
                ...
                print('Including the user_input_func in the pipeline! Please remember to return X, '
                      'and not modify the length or order of X at all. ')
                print('Your function will be called as the first step of the pipeline at both '
                      'training and prediction times. ')
                pipeline_list.append(('user_func',
                                      FunctionTransformer(
                                          func=self.user_input_func, validate=False)))

        # These parts will be included no matter what.
        if trained_pipeline is not None:
            pipeline_list.append(('basic_transform',
                                  trained_pipeline.named_steps['basic_transform']))
        else:
            pipeline_list.append(('basic_transform',
            ...
            if trained_pipeline is not None:
                ...
                if model_name[:12] == 'DeepLearning':
                    ...
                    pipeline_list.append(
                        ('scaler', utils_scaling.CustomSparseScaler(self.column_descriptions)))
        ...
        if trained_pipeline is not None:
            ...
            pipeline_list.append(('dv',
                                  DataFrameVectorizer.DataFrameVectorizer(
                                      sparse=True,
                                      column_descriptions=self.column_descriptions,
                                      keep_cat_features=keep_cat_features)))

        if self.perform_feature_selection is True:
            if trained_pipeline is not None:
                # This is the step we are trying to remove from the trained_pipeline, since it
                # has already been combined with dv using dv.restrict
                pass
            else:
                pipeline_list.append(('feature_selection',
                                      utils_feature_selection.FeatureSelectionTransformer(
                                          type_of_estimator=self.type_of_estimator,
                                          column_descriptions=self.column_descriptions,
                                          feature_selection_model='SelectFromModel')))

        if trained_pipeline is not None:
            # First, check and see if we have any steps with some version of keyword matching on
            # something like 'intermediate_model_predictions' or 'feature_learning_model' or
            ...
            # if it's none, then we've already added in the final model with our keyword matching
            # above!
            ...
            # Handling the case where we have run gscv on just the final model itself, and we now
            # need to integrate it back into the rest of the pipeline
            ...
            elif feature_learning is False:
            ...
            pipeline_list.append(('final_model',
                                  utils_model_training.FinalModelATC(
                                      model=final_model,
                                      type_of_estimator=self.type_of_estimator,
                                      ml_for_analytics=self.ml_for_analytics,
                                      name=self.name,
                                      _scorer=self._scorer,
                                      feature_learning=feature_learning,
                                      uncertainty_model=self.need_to_train_uncertainty_model,
                                      training_prediction_intervals=training_prediction_intervals,
                                      column_descriptions=self.column_descriptions,
        ...
        constructed_pipeline = utils.ExtendedPipeline(
            pipeline_list,
            keep_cat_features=keep_cat_features,
            name=self.name,
    ...
    def _get_estimator_names(self):
        if self.type_of_estimator == 'regressor':
            ...
            if self.compare_all_models is not True:
            ...
            raise TypeError('TypeError: type_of_estimator must be either "classifier" or '
                            '"regressor".')
    ...
    def _consolidate_pipeline(self, transformation_pipeline, final_model=None):
        ...
        # This has a number of benefits: speeds up computation, reduces memory usage,
    ...
    def set_params_and_defaults(self,
                                ...
                                train_uncertainty_model=None,
                                uncertainty_data=None,
                                uncertainty_delta=None,
                                uncertainty_delta_units=None,
        ...
        # If the user passed in a valid value for model_names (not None, and not a list where the
        # only thing is None)
        ...
        if 'DeepLearningRegressor' in self.model_names or 'DeepLearningClassifier' in self.model_names or feature_learning is True:
            if perform_feature_scaling is None or perform_feature_scaling is True:
                ...
                print(
                    ...
                    'essential for deep learning. We STRONGLY suggest not setting '
                    'perform_feature_scaling=False. ')
                warnings.warn('It is a best practice, and often necessary for training, '
                              'to perform_feature_scaling while doing Deep Learning. ')
        ...
        if self.uncertainty_delta_direction not in ['both', 'directional']:
            raise ValueError(
                'Please pass in either "both" or "directional" for uncertainty_delta_direction')

        if uncertainty_calibration_settings is None:
            self.uncertainty_calibration_settings = {'num_buckets': 10, 'percentiles': [25, 50, 75]}
        else:
        ...
        if skip_feature_responses is not True:
        ...
        if prediction_intervals is None:
            ...
            if isinstance(prediction_intervals, bool):
                # This is to allow the user to pass in their own bounds here, rather than having
                # to just use our 5% and 95% bounds
                self.calculate_prediction_intervals = prediction_intervals
            else:
                self.calculate_prediction_intervals = True

            if prediction_intervals is True:
                self.prediction_intervals = [0.05, 0.95]
            else:
                self.prediction_intervals = prediction_intervals

        self.train_uncertainty_model = train_uncertainty_model
        if self.train_uncertainty_model is True and self.type_of_estimator == 'classifier':
            print('!' * 64)
        ...
        if self.train_uncertainty_model is True and self.uncertainty_data is None:
            print('!' * 64)
        ...
        if self.feature_learning is True:
            if fl_data is None:
                print('!' * 64)
            ...
            if self.perform_feature_scaling is False:
                print('!' * 64)
                print('Heard that we should not perform feature_scaling, but we should perform '
                      ...
                      'perform_feature_scaling=False ')
                warnings.warn(
                    'Consider allowing auto_ml to perform_feature_scaling in conjunction with '
                    'feature_learning ')
            ...
            if self.perform_feature_selection is True:
                ...
                warnings.warn('perform_feature_selection=True is not currently supported with '
                              'feature_learning. ')
            ...
            if (isinstance(X_df, pd.DataFrame) and X_df.equals(fl_data)) or (isinstance(X_df, list)
                ...
                print(
                    ...
                    'training_data), and logistically (we remove the y column from both datasets, '
                    'which will throw an error) ')
                print('If you are looking for a quick and easy way of splitting the data, '
                      'use scikit-learn\'s train_test_split: df_train, fl_data = train_test_split('
                      'df_train, test_size=0.33) ')
                print('Or, if you insist on using the same dataset for both, you must at least '
                      'copy it: ')
                print(
                    'ml_predictor.train(df_train, feature_learning=True, fl_data=df_train.copy())')
                warnings.warn(
                    'Your fl_data and df_train must be different datasets. Use train_test_split, '
                    'or at least copy the data for your fl_data ')
        ...
        if trained_transformation_pipeline is None:
            ...
            print(
                'Be cautious when passing in a trained transformation pipeline- make sure that it '
                'is trained on exactly the same data. ')
    ...
    def fit_feature_learning_and_transformation_pipeline(self, X_df, fl_data, y):
        ...
        self.transformation_pipeline = self._construct_pipeline(
            trained_pipeline=self.transformation_pipeline,
            final_model=feature_learning_step,
    ...
    def train(self,
        ...
        self.set_params_and_defaults(
            raw_training_data,
            user_input_func=user_input_func,
            ...
            uncertainty_delta_units=uncertainty_delta_units,
            ...
            return_transformation_pipeline=return_transformation_pipeline,
            X_test_already_transformed=X_test_already_transformed,
            skip_feature_responses=skip_feature_responses,
            prediction_interval_params=prediction_interval_params)

        if verbose:
            print(
                'Welcome to auto_ml! We\'re about to go through and make sense of your data using '
        ...
        if self.X_test is not None and self.X_test_already_transformed is False:
        ...
        self.trained_pipeline = self._consolidate_pipeline(self.transformation_pipeline,
                                                           self.trained_final_model)

        # verify_features is not enabled by default. It adds a significant amount to the file
        # size of the saved pipelines. If you are interested in submitting a PR to reduce the
        # saved file size, there are definitely some optimizations you can make!
        if verify_features is True:
    ...
    def _create_uncertainty_model(self, uncertainty_data, scoring, y, uncertainty_calibration_data):
        ...
        # we just want it there for getting feature_names later for printing feature_importances
        ...
        if percent_uncertain == 1.0:
            ...
            warnings.warn(
                ...
                'predictions to train an uncertainty model. ')
            return self
        ...
        # base predictor's FinalModelATC (something like .trained_uncertainty_model). It's
        # important to grab this entire object, for all of the edge-case handling we've built in.
        self.trained_final_model.uncertainty_model = self.trained_uncertainty_model

        if self.calibrate_uncertainty is True:

            uncertainty_calibration_data_transformed = self.transformation_pipeline.transform(
                self.uncertainty_calibration_data)
            uncertainty_calibration_predictions = self.trained_final_model.predict_uncertainty(
                uncertainty_calibration_data_transformed)
            ...
            # If we have overlapping bucket definitions, pandas will drop those duplicates,
            # but won't drop the duplicate labels.
            # So we'll try bucketing one time, then get the actual number of bins from that
            try:
                bucket_results, bins = pd.qcut(
                    probas, q=num_buckets, retbins=True, duplicates='drop')
            except TypeError:
                bucket_results, bins = pd.qcut(probas, q=num_buckets, retbins=True)

            # now that we know the actual number of bins, we can create our labels, then use
            # those to create our final set of buckets.
            bucket_labels = range(1, len(bins))
            try:
                bucket_results = pd.qcut(
                    probas, q=num_buckets, labels=bucket_labels, duplicates='drop')
            except TypeError:
            ...
            for bucket in bucket_labels:
                dataset = uncertainty_calibration_predictions[
                    uncertainty_calibration_predictions['bucket_num'] == bucket]
            ...
            print('Here are the uncertainty_calibration results, for each bucket of predicted '
                  'probabilities ')
    ...
    def _prepare_for_verify_features(self):
        # Save the features we used for training to our FinalModelATC instance. This lets us
        # provide useful information to the user when they call .predict(data, verbose=True)
    ...
    def _calibrate_final_model(self, trained_model, X_test, y_test):
        ...
        print('Now calibrating the final model so the probability predictions line up with the '
              'observed probabilities in the X_test and y_test datasets you passed in. ')
        print('Note: the validation scores printed above are truly validation scores: they were '
              'scored before the model was calibrated to this data. ')
        print(
            'However, now that we are calibrating on the X_test and y_test data you gave us, '
            'it is no longer accurate to call this data validation data, since the model is being '
            'calibrated to it. As such, you must now report a validation score on a different '
            'dataset, or report the validation score used above before the model was calibrated '
            'to X_test and y_test. ')
        ...
        calibrated_classifier = CalibratedClassifierCV(
            trained_model, method=calibration_method, cv='prefit')
    ...
    def fit_single_pipeline(self,
        ...
        full_pipeline = self._construct_pipeline(
            model_name=model_name,
            feature_learning=feature_learning,
            prediction_interval=prediction_interval,
            keep_cat_features=self.transformation_pipeline.keep_cat_features)
        ...
        if self.verbose:
            ...
            if self.name is not None:
                print(self.name)
            if prediction_interval is not False:
                print('About to fit a {} quantile regressor to predict the prediction_interval for '
                      'the {}th percentile '.format(model_name, int(prediction_interval * 100)))
            else:
                print('About to fit the pipeline for the model ' + model_name + ' to predict ' +
                      self.output_column)
            print('Started at:')
        ...
        # Don't report feature_responses (or nearly anything else) if this is just the
        # feature_learning stage. That saves a considerable amount of time
        if feature_learning is False:
            self.print_results(model_name, ppl, X_df, y)

        return ppl

    # We have broken our model training into separate components. The first component is always
    # going to be fitting a transformation pipeline. The great part about separating the feature
    # transformation step is that now we can perform other work on the final step, and not have
    # to repeat the sometimes time-consuming step of the transformation pipeline.

    # NOTE: if included, we will be fitting a feature selection step here. This can get messy
    # later on with ensembling if we end up training on different y values.
    def fit_transformation_pipeline(self, X_df, y, model_names):

        keep_cat_features = True
        for model_name in model_names:
            keep_cat_features = keep_cat_features and model_name in [
                'LGBMRegressor', 'LGBMClassifier', 'CatBoostRegressor', 'CatBoostClassifier'
            ]

        self.keep_cat_features = keep_cat_features
        ppl = self._construct_pipeline(
            model_name=model_names[0], keep_cat_features=self.keep_cat_features)
        ppl.steps.pop()
    ...
    def create_feature_responses(self, model, X_transformed, y, top_features=None):
        ...
        # If we have fewer than 10000 rows, use all of them, regardless of user input. This
        # approach only works if there are a decent number of rows, so we will try to put some
        # safeguard in place to help the user from getting results that are too misleading
        row_multiplier = 1
        ...
        all_results = list(
            map(lambda tup: create_one_feature_response(col_idx=tup[0], col_name=tup[1]),
    ...
    def print_results(self, model_name, model, X, y):
        # This apparently fails in some cases. I'm not sure what those edge cases are, but the
        # try/except block should at least allow the rest of the script to continue
        try:
            if self.ml_for_analytics and model_name in ('LogisticRegression', 'RidgeClassifier',
                ...
                self._join_and_print_analytics_results(
                ...
                try:
                    ...
                    if self.skip_feature_responses is True:
                        feature_responses = None
                    else:
                        feature_responses = self.create_feature_responses(model, X, y, top_features)
                    self._join_and_print_analytics_results(
                        feature_responses, sorted_model_results, sort_field='Importance')
                ...
                feature_responses['FR_Incrementing_abs'] = np.absolute(
                    feature_responses.FR_Incrementing)
                feature_responses = feature_responses.sort_values(
                    by='FR_Incrementing_abs', ascending=False)
                feature_responses = feature_responses.reset_index(drop=True)
                feature_responses = feature_responses.head(n=100)
                feature_responses = feature_responses.sort_values(
                    by='FR_Incrementing_abs', ascending=True)
                feature_responses = feature_responses[[
                    'Feature Name', 'Delta', 'FR_Decrementing', 'FR_Incrementing', 'FRD_MAD',
                    'FRI_MAD'
                ]]
    ...
    def fit_grid_search(self, X_df, y, gs_params, feature_learning=False, refit=False):
        ...
        # Sometimes we're optimizing just one model, sometimes we're comparing a bunch of
        ...
        # We only want to run EASCV when we have more than 50 parameter combinations (it
        # efficiently searches very large spaces, but offers no benefits in small search spaces)
        ...
        # However, it does, itself, parallelize pretty nicely. So let lgbm take care of the
        ...
        if fit_evolutionary_search is True:
        ...
        if self.verbose:
            ...
            if self.optimize_final_model is True:
                ...
                if fit_evolutionary_search is False:
                    print('About to run GridSearchCV to find the optimal hyperparameters '
                          'for the model ' + model_name + ' to predict ' + self.output_column)
                else:
                    print('About to run EvolutionaryAlgorithmSearchCV to find the optimal '
                          'hyperparameters for the model ' + model_name + ' to predict ' +
                          self.output_column)
                    print('Population size each generation: ' + str(population_size))
                    print('Number of generations: ' + str(generations_number))
            else:
                print('About to run GridSearchCV on the pipeline for several models to predict ' +
                      self.output_column)
                # Note that we will only report analytics results on the final model that
                # ultimately gets selected, and trained on the entire dataset
    ...
    def create_gs_params(self, model_name):
        ...
        for param_name, param_list in raw_search_params.items():
            # We need to tell GS where to set these params. In our case, it is on the
            # "final_model" object, and specifically the "model" attribute on that object
        ...
        if self.user_gs_params is not None:
            ...
            print('Please note that if you want to set the grid search params for the final model '
                  'specifically, they need to be prefixed with: "model__" ')

        return grid_search_params

    # When we go to perform hyperparameter optimization, the hyperparameters for a
    # GradientBoosting model will not at all align with the hyperparameters for an SVM. Doing all
    # of that in one giant GSCV would throw errors. So we train each model in it's own grid search.
    # TODO: Simplify
    def train_ml_estimator(self,
        ...
        elif (feature_learning is True and self.optimize_feature_learning is not True) or (
                len(estimator_names) == 1 and self.optimize_final_model is not True):
            trained_final_model = self.fit_single_pipeline(
                X_df,
                y,
                estimator_names[0],
        ...
        elif len(estimator_names) > 1 and self.optimize_final_model is not True:
            grid_search_params = {}
        ...
        # Use Case 4: Many models, and optimize them!
        elif (feature_learning is False and self.optimize_final_model is True) or (
                feature_learning is True and self.optimize_feature_learning is True):
            ...
            for model_name in estimator_names:
                ...
                gscv_results = self.fit_grid_search(
                    X_df, y, grid_search_params, feature_learning=feature_learning)
            ...
            # Now that we've got the best model, train it on quite a few more
            # iterations/epochs/trees if applicable
            cleaned_best_params = {}
            ...
            trained_final_model = self.fit_single_pipeline(
                X_df, y, model_name, feature_learning=feature_learning, prediction_interval=False)

            # Don't report feature_responses (or nearly anything else) if this is just the
            # feature_learning stage. That saves a considerable amount of time if not
            # feature_learning: self.print_results(model_name, trained_final_model, X_df, y)

            # If we wanted to do something tricky, here would be the place to do it. Train the
    ...
    def train_categorical_ensemble(self,
        ...
        print('Now fitting a single feature transformation pipeline that will be shared by all of '
              'our categorical estimators for the sake of space efficiency when saving the model ')
        if self.feature_learning is True:
            # For simplicity's sake, we are training one feature_learning model on all of the
            # data, across all categories.
            # Deep Learning models love a ton of data, so we're giving all of it to the model.
            # This also makes stuff like serializing the model and the transformation pipeline
            # and the saved file size all better.
            # Then, each categorical model will determine which features (if any) are useful for
            # it's particular category.
            X_df_transformed = self.fit_feature_learning_and_transformation_pipeline(
                X_df, kwargs['fl_data'], y)
        else:
            # If the user passed in a valid value for model_names (not None, and not a list where
            # the only thing is None)
            if self.model_names is not None and not (len(self.model_names) == 1
                                                     and self.model_names[0] is None):
                estimator_names = self.model_names
            else:
        ...
        # 2. some sorting (either alphabetical, size of category, or ideally, sorted by magnitude
        # of y value).

        # 3. size of category would be most efficient. if we have 8 cores and 13 categories,
        # we don't want to save the largest category for the last one, even if from an analytics
        # perspective that's the one we would want last

        # 4. create a map from category name to indexes

        # 5. sort by len(indices)

        # 6. iterate through that, creating a new mapping from category_name to the relevant data
        # for that category. pull that data from X_df_transformed
        ...
        categories_and_data = []
        all_small_categories = {'relevant_transformed_rows': [], 'relevant_y': []}
        for pair in sorted(categories_and_indices, key=lambda x: len(x[1]), reverse=True):
            ...
            # Otherwise, add it to our "all_small_categories" category, and train a model on all
            # our small categories combined
            else:
        ...
        if len(all_small_categories['relevant_y']) > self.min_category_size:
            categories_and_data.insert(0, [
                '_all_small_categories', all_small_categories['relevant_transformed_rows'],
                all_small_categories['relevant_y']
            ])
        ...
        if os.environ.get('is_test_suite', False) == 'True':
            ...
            results = list(
                map(lambda x: train_one_categorical_model(x[0], x[1], x[2]), categories_and_data))
        else:
            ...
            try:
                results = list(
                    pool.map(lambda x: train_one_categorical_model(x[0], x[1], x[2]),
                             categories_and_data))
            except RuntimeError:
                # Deep Learning models require a ton of recursion. I've tried to work around it,
                # but sometimes we just need to brute force the solution here
        ...
        if self.search_for_default_category is True:
            print(
                'By default, auto_ml finds the largest category, and uses that if asked to get '
                ...
                'market that was not included in the training data ')
            print('To avoid this behavior, you can either choose your own default category (the '
                  '"default_category" parameter to train_categorical_ensemble), or pass in '
                  '"_RAISE_ERROR" as the value for default_category, and we will raise an error '
                  'when trying to get predictions for a row coming from a category that was not '
                  'included in the training data. ')
            print('\n\nHere is the default category we selected:')
            print(self.default_category)
            if self.default_category == '_all_small_categories':
                print('In this case, it is all the categories that did not meet the '
                      'min_category_size threshold, combined together into their own '
                      '"_all_small_categories" category. ')

        categorical_ensembler = utils_categorical_ensembling.CategoricalEnsembler(
            self.trained_category_models, self.transformation_pipeline, self.categorical_column,
    ...
    def _join_and_print_analytics_results(self, df_feature_responses, df_features, sort_field):
        ...
        # Join the standard feature_importance/coefficients, with our feature_responses
        if df_feature_responses is not None:
            df_results = pd.merge(df_feature_responses, df_features, on='Feature Name')

            # Sort by coefficients or feature importance
            df_results = df_results[[
                'Feature Name', sort_field, 'Delta', 'FR_Decrementing', 'FR_Incrementing',
                'FRD_abs', 'FRI_abs', 'FRD_MAD', 'FRI_MAD'
            ]]
        else:
        ...
        # print('The full analytics results will be saved to a filed called: ' +
        # analytics_file_name + '\n')
        ...
        print('Explanation: A weighted measure of how much of the variance the model is able to '
        ...
        print('FR_Incrementing = Feature Response From Incrementing Values In This Column By One '
              'FR_delta ')
        print('Explanation: Represents how much the predicted output values respond to adding one '
              'FR_delta amount to every value in this column ')
        print('FRD_MAD = Feature Response From Decrementing- Median Absolute Delta')
        print('Explanation: Takes the absolute value of all changes in predictions, then takes the '
              'median of those. Useful for seeing if decrementing this feature provokes strong '
              'changes that are both positive and negative ')
        print('FRI_MAD = Feature Response From Incrementing- Median Absolute Delta')
        print('Explanation: Takes the absolute value of all changes in predictions, then takes the '
              'median of those. Useful for seeing if incrementing this feature provokes strong '
              'changes that are both positive and negative ')
        print('FRD_abs = Feature Response From Decrementing Avg Absolute Change')
        print('Explanation: What is the average absolute change in predicted output values to '
              'subtracting one FR_delta amount to every value in this column. Useful for seeing if '
              'output is sensitive to a feature, but not in a uniformly positive or negative way ')
        print('FRI_abs = Feature Response From Incrementing Avg Absolute Change')
        print('Explanation: What is the average absolute change in predicted output values to '
              'adding one FR_delta amount to every value in this column. Useful for seeing if '
              'output is sensitive to a feature, but not in a uniformly positive or negative way ')
        print('*' * 64 + '\n')
    ...
    def print_training_summary(self, gs):
        print(
            'The best CV score from our hyperparameter search (by default averaging across k-fold '
            'CV) for ' + self.output_column + ' is:')
        ...
        # Remove 'final_model__model' from what we print- it's redundant with model name,
        # and is difficult to read quickly in a list since it's a python object.
        printing_copy = {}
        ...
        if self.verbose:
            # sklearn-deap has an annoying bug where it doesn't keep .cv_results_ for the first
            # generation, and the second generation might be empty when running the test suite
            # because we have such a small number per generation.
            try:
            ...
            if self.type_of_estimator == 'regressor':
                ...
                if self._scorer == accuracy_score:
                    ...
                    score, probas = self._scorer.score(
                        self.trained_pipeline, X_test, y_test, advanced_scoring=advanced_scoring)
        ...
        for idx, y_val in enumerate(y):
            ...
            if self.uncertainty_delta_units == 'absolute':
                if self.uncertainty_delta_direction == 'both':
                    ...
                    # This is now the case of single-directional deltas (we only care if our
                    # predictions are higher, not lower, or lower and not higher)
                    if self.uncertainty_delta > 0:
                        ...
                        # This is the case where we have directional deltas, and the
                        # uncertainty_delta < 0
                        if delta < self.uncertainty_delta:
                            is_uncertain_predictions.append(1)
                        else:
    ...
    def score_uncertainty(self, X, y, advanced_scoring=True, verbose=2):
        ...
        is_uncertain_predictions = self.define_uncertain_predictions(
            df_uncertainty_predictions.base_prediction, y)

        score = utils_scoring.advanced_scoring_classifiers(
            df_uncertainty_predictions.uncertainty_prediction, is_uncertain_predictions)
    ...
    def save(self, file_name='auto_ml_saved_pipeline.dill', verbose=True):
        ...
        try:
            # CategoricalEnsembler doesn't have named_steps, and Ensembler doesn't have .model,
            # so we perform some conditional logic here to add feature_importances_ whenever we can
            feature_names = self.trained_pipeline.named_steps['dv'].get_feature_names()
        except AttributeError:
            make_feature_importances = False

        if make_feature_importances is True:
            importances_dict = {}
            try:
        ...
        def save_one_step(pipeline_step, used_deep_learning):
            try:
                if pipeline_step.model_name[:12] == 'DeepLearning':
                    ...
                    # Now that we've saved the keras model, set that spot in the pipeline to our
                    # random name, because otherwise we're at risk for recursionlimit errors (the
                    # model is very recursively deep) Using the random_name allows us to find
                    # the right model later if we have several (or several thousand) models to
        ...
        # This is where we will store all of our Keras models by their name, so we can put them
        # back in place once we've taken them out and saved the rest of the pipeline
        model_name_map = {}
        ...
        # Now, whether we had deep learning models in there or not, save the structure of the
        # whole pipeline. We've already removed the deep learning models from it if they existed,
        # so they won't be throwing recursion errors here
        with open(file_name, 'wb') as open_file_name:
            dill.dump(self.trained_pipeline, open_file_name)

        # If we used deep learning, put the models back in place, so the predictor instance
        # that's already loaded in memory will continue to work like the user expects (rather
        # than forcing them to load it back in from disk again)
        if used_deep_learning is True:
            if isinstance(self.trained_pipeline, utils_categorical_ensembling.CategoricalEnsembler):
                for step in self.trained_pipeline.transformation_pipeline.named_steps:
                    pipeline_step = self.trained_pipeline.transformation_pipeline.named_steps[step]
        ...
        if verbose:
            ...
            if used_deep_learning is True:
                print('While saving the trained_ml_pipeline, we found a number of deep learning '
                      'models that we saved separately. ')
                print('Make sure to transfer these files to whichever environment you plan to load '
                      'the trained pipeline in ')
                print('Specifically, we saved ' + str(len(model_name_map.keys())) +
                      ' deep learning models to separate files')

            print('Note that this pickle/dill file can only be loaded in an environment with the '
                  'same modules installed, and running the same Python version. ')
            print('This version of Python is:')
            print(sys.version_info)

            print('\n\nWhen passing in new data to get predictions on, columns that were not '
                  'present (or were not found to be useful) in the training data will be silently '
                  'ignored. ')
            print('It is worthwhile to make sure that you feed in all the most useful data points '
                  'though, to make sure you can get the highest quality predictions. ')
    ...
    def _train_ensemble(self, X_train, y_train):
        ...
        if ensemble_method != 'average' and self.type_of_estimator == 'classifier':
            print('Because we are taking the minimum prediction for each class, these predicted '
                  'probabilities are not expected to sum to 1 for each row ')
            print('If you want the predicted probabilities to sum to 1, you should use '
                  'ensemble_method="average" ')
            warnings.warn(
                'Predicted probabilities are not expected to add up to 1 if ensemble_method is '
                'not "average" ')
        ...
        # create Ensembler
        ensembler = utils_ensembling.Ensembler(
            ensemble_predictors=trained_ensemble_models,
            type_of_estimator=self.type_of_estimator,
