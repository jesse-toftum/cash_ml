...
class RegressionScorer(object):
    ...
    def score(self,
        ...
        try:
            ...
            print('Found ' + str(len(bad_val_indices)) +
                  'null or infinity values in the predicted or y values. We '
                  'will ignore these, and report the score on the rest of the '
                  'dataset')
            score = self.scoring_func(y, predictions)
...
class ClassificationScorer(object):
    ...
    def clean_probas(self, probas):
        print('Warning: We have found some values in the predicted probabilities that fall outside '
              'the range {0, 1}')
        print(
            'This is likely the result of a model being trained on too little data, or with a bad '
            'set of hyperparameters. If you get this warning while doing a hyperparameter search, '
    ...
    def score(self, estimator, X, y, advanced_scoring=False):
        ...
        try:
            ...
            print('Found ' + str(len(bad_val_indices)) +
                  'null or infinity values in the y values. We will ignore '
                  'these, and report the score on the rest of the dataset')
            try:
                score = self.scoring_func(y, predictions)
            except ValueError:
