...
bad_vals_as_strings = {
    str(float('nan')),
    str(float('inf')),
    str(float('-inf')), 'None', 'none', 'NaN', 'nan', 'NULL', 'null', '', 'inf', '-inf'
}
...
# This plays more nicely with df.apply, and assumes we will be handling nans appropriately when
# doing DataFrameVectorizer later.
...
class BasicDataCleaning(BaseEstimator, TransformerMixin):
        ...
        for key, val in self.column_descriptions.items():
            if val in self.text_col_indicators:
                self.text_columns[key] = TfidfVectorizer(
                    # If we have any documents that cannot be decoded properly, just ignore them
                    # and keep going as planned with everything else
                    decode_error='ignore'
                    # Try to strip accents from characters. Using unicode is slightly slower but
                    # more comprehensive than 'ascii'
                    ,
                    strip_accents='unicode'
                    # Can also choose 'character', which will likely increase accuracy, at the
                    # cost of much more space, generally
                    ,
                    analyzer='word'
                    # Remove commonly found english words ('it', 'a', 'the') which do not
                    # typically contain much signal
                    ,
                    stop_words='english'
                    ...
                    ,
                    max_df=0.9
                    # Consider only the most frequently occurring 3000 words, after taking into
                    # account all the other filtering going on
                    ,
                    max_features=3000)
    ...
    def fit(self, X_df, y=None):
        ...
        for key in X_df.columns:
            ...
            if X_df[key].dtype == 'object' and self.column_descriptions.get(
                    key, False) not in ['categorical', 'ignore', 'nlp']:
                ...
                if is_categorical:
                    print('\n')
                    print('Encountered a column that is not marked as categorical, but is an '
                          '"object" pandas type, which typically indicates a categorical column. ')
        ...
        if isinstance(X, dict):
            ...
            for key, val in X.items():
                ...
                if col_desc is None:
                    ...
                    # Then we zip together the relevant columns and the sparse data into a
                    # dictionary
            ...
            if len(cols_to_clean) > 0:
                ...
                if df_to_clean.shape[0] > 100000 or os.environ.get('is_test_suite', 0) == 'True':
                    results = list(map(
                        lambda col: self.process_one_column(column_values=df_to_clean[col],
                                                            col_name=col), df_to_clean.columns))
                else:
    ...
    # TODO: Simplify
    def process_one_column(self, column_values, col_name):
        column_descriptions = self.get('transformed_column_descriptions', self.column_descriptions)

        col_desc = column_descriptions.get(col_name)

        # This is what we do to columns that were not present at fitting time. All columns that
        # were present at fitting time that had no entry in column_descriptions were filled in
        # with 'continuous'
        if col_desc is None:
            result = {}

        elif col_desc == 'categorical':
            # We will handle categorical data later, one-hot-encoding it inside
            # DataFrameVectorizer (or LabelEncoding it for lgbm)
            result = {col_name: column_values}
            ...
            result = add_date_features_df(column_values, col_name)
            ...
            if pandas_version < '0.20.0':
                nlp_matrix = self.text_columns[col_name].transform(
                    column_values.astype(str, raise_on_error=False))
            else:
                nlp_matrix = self.text_columns[col_name].transform(
                    column_values.astype(str, errors='ignore'))
            ...
            result = {}
            for column_values in text_df.columns:
                result[column_values] = text_df[column_values].astype(int)
            ...
            print(col_desc)
            warnings.warn('UnknownValueInColumnDescriptions: Please make sure all the values you '
                          'pass into column_descriptions are valid.')
            result = {}

        # TODO: Make sure that result is initialized
        return result
...
def add_date_features_df(col_data, date_col):
    ...
    if pandas_version < '0.20.0':
        result[date_col + '_day_of_week'] = col_data.apply(lambda x: x.weekday()).astype(
            int, raise_on_error=False)
    else:
        result[date_col + '_day_of_week'] = col_data.apply(lambda x: x.weekday()).astype(
            int, errors='ignore')
    ...
    try:
        if pandas_version < '0.20.0':
            result[date_col + '_hour'] = col_data.apply(lambda x: x.hour).astype(
                int, raise_on_error=False)
        else:
            result[date_col + '_hour'] = col_data.apply(lambda x: x.hour).astype(
                int, errors='ignore')
    ...
    result[date_col + '_day_part'] = result[date_col +
                                            '_minutes_into_day'].apply(minutes_into_day_parts)
...
# Same logic as above, except implemented for a single dictionary, which is much faster at
# prediction time when getting just a single prediction
