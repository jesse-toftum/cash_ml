...
class CategoricalEnsembler(object):
    ...
    def predict(self, data):
        # For now, we are assuming that data is a list of dictionaries, so if we have a single
        # dict, put it in a list
...
def clean_categorical_definitions(df, categorical_column):
    ...
    if sum_of_nan_values > 0:
        ...
        print('We will default to making these values a string "nan" instead, since that can be '
              'used as a key ')
        print('If this is not the behavior you want, consider changing these categorical_column '
              'values yourself ')
